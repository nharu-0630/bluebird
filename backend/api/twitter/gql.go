...
func (c *Client) TweetByID(id string) (model.Tweet, error) {
	zap.L().Debug(TweetByID.Name, zap.String("id", id))
	params := TweetByID.DefaultParams
	params["variables"].(map[string]interface{})["tweetId"] = id
	res, err := c.gql("GET", TweetByID.Endpoint, TweetByID.Name, params)
	tools.LogRaw([]string{TweetByID.Name, id}, res, false)
	if err != nil {
		return model.Tweet{}, err
	}
	return ParseTweet(TweetByID.Params(res))
}
